...
export default function AgentPanel({
  ...
  return (
    <div className={`p-5 rounded-xl border h-full flex flex-col justify-between transition-all duration-300 ${isMatrix
      ...
      <div>
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-dashed border-slate-700/50">
          ...
          <button
            ...
            disabled={isBusy}
            className={`flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-md border tracking-wider transition-colors uppercase ${isBusy
        ...
        {showAddForm && !isBusy ? (
          <form onSubmit={handleCreateAgent} className={`space-y-3 mb-4 p-3.5 rounded-lg border border-dashed ${isMatrix
            ? "bg-black border-green-500/30 text-green-400"
            : isNight
              ? "bg-slate-900/40 border-slate-700/50 text-slate-100"
              : "bg-slate-50 border-slate-200 text-slate-900"
            }`}>
            <h3 className={`text-xs font-bold uppercase tracking-wider ${isMatrix ? "text-green-400 font-mono" : isNight ? "text-indigo-400" : "text-indigo-900"
              }`}>Spawn Dynamic Agent</h3>
            <div>
              <label className={`block text-[10px] uppercase font-bold mb-1 ${isMatrix ? "text-green-500 font-mono" : isNight ? "text-slate-400" : "text-slate-800 font-extrabold"
              ...
              <input
                ...
                className={`w-full px-2.5 py-1.5 text-xs rounded border focus:outline-none focus:border-indigo-500 ${isMatrix ? "bg-black border-green-500/30 text-green-400 font-mono" : isNight ? "bg-slate-950/80 border-slate-700 text-white" : "bg-white border-slate-200 text-slate-900 font-medium"
                  }`}
              />
            </div>
            <div>
              ...
              <div className="flex gap-2 py-1 items-center">
                {PALETTE_COLORS.map((c) => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-full border transition-transform ${newAgentColor === c.hex ? "scale-125 border-white ring-2 ring-indigo-500" : "border-transparent"
                      }`}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`py-1.5 px-2.5 text-[10px] rounded border transition-all font-bold uppercase ${newAgentGender === "male"
                    ? isMatrix
                ...
                <button
                  ...
                  className={`py-1.5 px-2.5 text-[10px] rounded border transition-all font-bold uppercase ${newAgentGender === "female"
                    ? isMatrix
                      ? "bg-green-600 border-green-500 text-black font-mono"
                      : "bg-indigo-600 border-indigo-500 text-white shadow-sm"
                    : isMatrix
            ...
            <div>
              ...
              <textarea
                ...
                className={`w-full px-2.5 py-1.5 text-xs rounded border focus:outline-none focus:border-indigo-500 font-mono ${isMatrix ? "bg-black border-green-500/30 text-green-400" : isNight ? "bg-slate-950/80 border-slate-700 text-white" : "bg-white border-slate-200 text-slate-900 font-medium"
                  }`}
              />
            </div>
            <div className="flex gap-2 justify-end pt-1">
              <button
                ...
                className={`px-2 py-1 text-[10px] rounded hover:underline ${isMatrix ? "text-green-400" : isNight ? "text-slate-400" : "text-slate-600 font-extrabold"
                  }`}
              >
                Cancel
              </button>
        ...
        {selectedAgent ? (
          <form onSubmit={handleSaveEdit} className="space-y-4">
            {isBusy ? (
              <div className={`p-3 rounded-lg flex items-start gap-2.5 border ${isMatrix
              ...
              <div className={`p-3.5 rounded-lg flex items-start gap-2.5 border ${isMatrix
            ...
            <div className="space-y-3">
              <div>
                <label className={`block text-[10px] uppercase font-extrabold mb-1 ${isMatrix ? "text-green-500" : isNight ? "text-slate-400" : "text-slate-900 text-black"
                  } ${isBusy ? "opacity-60" : ""}`}>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 text-xs rounded border focus:outline-none focus:border-indigo-500 ${isBusy ? "bg-slate-900/60 border-slate-800 text-slate-500 cursor-not-allowed" :
                ...
                <div className="flex gap-2 py-1 items-center">
                  {PALETTE_COLORS.map((c) => (
                    <button
                      ...
                      className={`w-6 h-6 rounded-full border transition-transform ${isBusy ? "cursor-not-allowed opacity-50" : ""
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`py-1.5 px-3 text-xs rounded border transition-all font-semibold uppercase ${isBusy ? "cursor-not-allowed opacity-50" : ""
                      } ${gender === "male"
                        ? isMatrix
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full p-2.5 text-xs rounded border focus:outline-none focus:border-indigo-500 font-mono leading-relaxed ${isBusy ? "bg-slate-900/60 border-slate-800 text-slate-500 cursor-not-allowed" :
            ...
            <div className="flex gap-2 items-center justify-between pt-1">
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-md border text-[11px] font-bold uppercase transition-colors ${isBusy || agents.length <= 1
              ...
              <button
                type="submit"
                disabled={isBusy}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-md font-bold text-[11px] uppercase transition-all ${isBusy ? "opacity-50 cursor-not-allowed bg-slate-800 border-slate-755 text-slate-400" :
                  isMatrix
